...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210512181225703.png" alt="image-20210512181225703" style="zoom:50%;" />
...
func del(head *ListNode, target int) *ListNode{
	if head == nil{
		return nil
	}
	
	if head.Val == target{
		return del(head.Next, target)
	}else{
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210508152840116.png" alt="image-20210508152840116" style="zoom:50%;" />
...
<img src="201-300.assets/image-20210625092242578.png" alt="image-20210625092242578" style="zoom:50%;" />

```cpp
class Solution {
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        vector<int>       prerequisiteCourse(numCourses, 0);
        vector<set<int>>  outDegree(numCourses, set<int>());

        for(vector<int>& v : prerequisites){
            prerequisiteCourse[v[0]] += 1;
            outDegree[v[1]].insert(v[0]);
        }
...
<img src="201-300.assets/image-20210921151508616.png" alt="image-20210921151508616" style="zoom:50%;" />
...
class Node{
 public:
    ...
    Node(char c) : ch(c), word(false), children(vector<Node*>(26, nullptr)){}
};
...
public:
    ...
    bool dfs(string& word, Node* cur, int progress){
        if(progress == word.size() - 1){
            char ch = word[progress];
            if(ch == '.'){
                for(int i = 0; i < 26; i++)
                    if(cur->children[i] != nullptr &&  cur->children[i]->word)
                        return true;
                return false;
            }else{
                return (cur->children[ch - 'a'] != nullptr && cur->children[ch - 'a']->word);
            }
        }
...
//                    cout << cur-> children[i]->ch << endl;
                    if(dfs(word, cur->children[i], progress + 1))
                        return true;
                }
            }
            ...
            if(cur->children[ch - 'a'] == nullptr)
                return false;
            cur = cur->children[ch - 'a'];
            return dfs(word, cur, progress + 1);
        }
    }
};
...
<img src="201-300.assets/image-20210930155826698.png" alt="image-20210930155826698" style="zoom:50%;" />
...
public:
    int computeArea(int ax1, int ay1, int ax2, int ay2, int bx1, int by1, int bx2, int by2) {
        ...
        return a1 + a2 - abs(x1 - x2) * abs(y1 - y2);
    }
};
```
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210508143020722.png" alt="image-20210508143020722" style="zoom:50%;" />
...
<img src="201-300.assets/image-20210801164942328.png" alt="image-20210801164942328" style="zoom:50%;" />
...
<img src="201-300.assets/image-20210805172513967.png" alt="image-20210805172513967" style="zoom:50%;" />

```cpp
//source : https://leetcode.wang/leetcode-233-Number-of-Digit-One.html
class Solution {
public:
...
```cpp
/*
		ref: https://leetcode-cn.com/problems/group-shifted-strings/solution/c-ha-xi-biao-shi-xian-yong-shi-4ms-ji-bai-9661-nei/
		本质上是把所有字符串转移到 以a 开头， 对齐然后比较
*/
class Solution {
public:
    vector<vector<string>> groupStrings(vector<string>& strings) {
        ...
        for(int i = 0; i < size; i++){
            ...
            if(map.count(ss) != 0){
                int pos = map[ss];
                res[pos].emplace_back(strings[i]);
            }else{
                map.insert({ss, res.size()});
...
<img src="201-300.assets/image-20210708080554513.png" alt="image-20210708080554513" style="zoom:50%;" />
...
public:
    bool canAttendMeetings(vector<vector<int>>& intervals) {
        ...
        sort(intervals.begin(), intervals.end(), [](vector<int>& v1, vector<int>& v2){ return v1[0] == v2[0] ? v1[1] > v2[1] : v1[0] < v2[0];});
...
public:
    bool verifyPreorder(vector<int>& preorder) {
        ...
        for(int i = 0; i < size; i++){
            ...
            while(!myStack.empty() && preorder[i] > preorder[myStack.top()]){
                curMin = max(curMin, preorder[myStack.top()]);
                myStack.pop();
            }
...
<img src="201-300.assets/image-20210806153709842.png" alt="image-20210806153709842" style="zoom:50%;" />
...
//                cout << firstIndex << " " << secondIndex << endl;
                ...
                int tempFirstMin = INT_MAX;
                int tempFirstIndex = -1;
                int tempSecondMin = INT_MAX;
                int tempSecondIndex = -1;
                for(int j = 0; j < colors; j++){
                    int curCost = costs[i][j] + (firstIndex == j ? secondMin : firstMin);
                    if(tempFirstMin == INT_MAX){
                        tempFirstMin = curCost;
                        tempFirstIndex = j;
                    }
                    else if(curCost < tempFirstMin){
                        tempSecondMin = tempFirstMin;
                        tempFirstMin = curCost;
                        tempSecondIndex = tempFirstIndex;
                        tempFirstIndex = j;
                    }else if(curCost < tempSecondMin){
                        tempSecondMin = curCost;
                        tempSecondIndex = j;
                    }
                }
...
<img src="201-300.assets/image-20210917161501063.png" alt="image-20210917161501063" style="zoom:50%;" />
...
public:
    string alienOrder(vector<string>& words) {
        ...
        for(int i = 0; i < size - 1; i++){
            ...
            for(int j = 0; j < minSize; j++){
                if(curStr[j] != nextStr[j]){
                    map[curStr[j]].push_back(nextStr[j]);
                    indegree[nextStr[j] - 'a']++;
                    break;
                }
            }
        }
        ...
        deque<char> myQueue;
        for(int i = 0; i < 26; i++){
            if(hasAlpha[i] != 0 && indegree[i] == 0){
//                cout << (char)(i + 'a') << endl;
                myQueue.push_back((char)(i + 'a'));
            }
        }

        vector<bool> hasHad(26, false);
        string res;
        while(!myQueue.empty()){
            int size = myQueue.size();
            for(int i = 0; i < size; i++){
    ...
    string alienOrder(vector<string>& words) {
        ...
        for(int i = 0; i < size - 1; i++){
            ...
            for(int j = 0; j < words[i].size() && j < words[i + 1].size(); j++){
                ...
                map[words[i][j]].insert(words[i + 1][j]);
                break;
            }
        }

        vector<int> degree = vector<int>(26, -1);
        ...
        while(!myQueue.empty()){
            char ch = myQueue.front(); myQueue.pop();
...
```cpp
/*
	ref: https://leetcode-cn.com/problems/expression-add-operators/solution/xiang-xi-tong-su-de-si-lu-fen-xi-duo-jie-fa-by-52/
*/
class Solution {
public:
    vector<string> res;
    ...
    void helper(string& num, int target, string path, int start, long eval, long pre){
        if(start == num.size()){
            if(eval == target){
                res.push_back(path);
            }
        ...
        for(int i = start; i < num.size(); i++){
            ...
            if(start == 0){
                helper(num, target, path + to_string(cur), i + 1, cur, cur);
            }else{
                
                helper(num, target, path + "+" + to_string(cur), i + 1, eval + cur, cur);
                helper(num, target, path + "-" + to_string(cur), i + 1, eval - cur, -cur);
                helper(num, target, path + "*" + to_string(cur), i + 1, eval - pre + pre * cur, pre * cur);
...
<img src="201-300.assets/image-20210714091634493.png" alt="image-20210714091634493" style="zoom:50%;" />
...
public:
    bool wordPattern(string pattern, string s) {
        ...
        for(int i = 0; i < pattern.size(); i++){
            char ch = pattern[i];
            if(hasMapped.count(vec[i]) != 0){
                if(m1.count(ch) == 0 || m1[ch] != vec[i])
                    return false;
            }else{
                if(m1.count(ch) != 0)
                    return false;

                m1.emplace(make_pair(ch, vec[i]));
                hasMapped.emplace(vec[i]);
            }
        }
...
func wordPattern(pattern string, s string) bool {
	...
	for ; index1 != len(pattern) && index2 != len(words); {
		ch := pattern[index1]
	...
	return index1 == len(pattern) && index2 == len(words)
}
```
...
<img src="201-300.assets/image-20210802064238193.png" alt="image-20210802064238193" style="zoom:50%;" />
...
public:
    ...
    bool dfs(string pattern, string s, int index1, int index2){
        if(index1 == pattern.size() || index2 == s.size()){
            if(index1 == pattern.size() && index2 == s.size())
                return true;
            return false;
        }

        char frac1 = pattern[index1];
        for(int j = index2 + 1; j <= s.size(); j++){
            string frac2 = s.substr(index2, j - index2);

            if(map1.count(frac1) == 0){
                if(map2.count(frac2) != 0)
                    continue;
                map1.insert({frac1, frac2});
                map2.insert({frac2, frac1});

                if(dfs(pattern, s, index1 + 1, j))
                    return true;
...
public:
    int minTotalDistance(vector<vector<int>>& grid) {
        ...
        for(pair<int, int>& p : vec){
            int xDiff = std::abs(p.first - midX);
            int yDiif = std::abs(p.second - midY);
