...
![image-20200817105019394](301-400.assets/image-20200817105019394.png)
...
![image-20200817104332379](301-400.assets/image-20200817104332379.png)
...
class TicTacToe {
    ...
    public int move(int row, int col, int player) {
        ...
        if(player == 1)     setA.add(row + "@" + col);
        else                setB.add(row + "@" + col);

        int status = checkIfWin(row, col, player);
        return status;

    }

    private int checkIfWin(int row, int col, int player)
    {
        HashSet<String> set = player == 1 ? setA : setB;
        //check horizontally
        for(int j = 0; j <= board[0].length; j++)
        {
            if(j == board[0].length)       return player;

            if(!set.contains(row + "@" + j))  break;
        }
        //check vertically
        ...
        for(int i = board.length - 1; i >= -1; i--)
        {
            ...
            if(!set.contains(board.length - 1 - i + "@" + i))    break;
        }
...
![image-20200817145348440](301-400.assets/image-20200817145348440.png)



# 348 Tic Tac Toe 下午完成
...
# 358 Rearrange String k Distance Apart 贪心算法 未完成
...
```java
/*
		Full Code for this question
		Adapted from Labuladong
*/
class Solution {
    int base = 1337;
    ...
    public int superPow(int a, ArrayDeque<Integer> queue) {
...
# 375 Guess Number Higher or Lower II 典型 DP 题目 未解决
...
public int wiggleMaxLength(int[] nums) {
  ...
  //dpNeg：以nums[i] 为结尾， 与前面单位差值为负， 的最大大原属
