...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        cout << "? " << endl;
        while(true){
            int tempIndex = parition(vec, left, right);
            cout << tempIndex << " " << targetIndex << endl;
    ...
    int parition(vector<Node>& vec, int lo, int hi){
        ...
        int i = lo, j = hi + 1;
        int pivot = vec[lo].freq;

        while(true){
            while(vec[++i].freq < pivot) if(i == hi) break;
            while(vec[--j].freq > pivot) if(j == lo) break;
...
```cpp
/*
		优化解法 思路来源https://www.youtube.com/watch?v=RZVPX3elY9I
...
j=> X X X X X X X X X
    ...
    通过前缀和 + set 的 lower_bound 找到对应目标值
...
public:
    int maxSumSubmatrix(vector<vector<int>>& matrix, int k) {
        ...
        if(row > col){
            auto matrix2 = vector<vector<int>>(col, vector<int>(row, 0));
            for(int i = 0; i < row; i++){
                for(int j = 0; j < col; j++)
    ...
    int helper(vector<int>& row, int k){
        ...
        for(int i = 0; i < row.size(); i++){
            ...
            auto it = mySet.lower_bound(prefixSum - k);
            if(it != mySet.end())
...
<img src="301-400.assets/image-20210514161158545.png" alt="image-20210514161158545" style="zoom:50%;" />
...
public:
    ...
    vector<int> shuffle() {
        ...
        for(int i = 0; i < nums.size(); i++){
            if(i == nums.size() - 1)
                continue;
            int pos = std::rand() % (nums.size() - i) + i; 
            swap(res, i, pos);
        }
...
<img src="301-400.assets/image-20210329162142581.png" alt="image-20210329162142581" style="zoom:50%;" />
   ...
   class Node{
    ...
    public boolean isRectangleCover(int[][] rectangles) {
        ...
        for(int[] rect : rectangles){
            ...
            addNodeToMap(map, rect[0], rect[2], rect[3], rect[1]);
        }

        if(area != (right - left) * (top - bottom))
            return false;

        addNodeToMap(map, left, right, top, bottom);
    ...
    private void addNodeToMap(Map<Node, Integer> map, int left, int right, int top, int bottom){
        Node leftBtn  = new Node(left, bottom);
        Node rightTop = new Node(right, top);
        Node leftTop  = new Node(left, top);
        Node rightBtn = new Node(right, bottom);

        map.put(leftBtn, map.getOrDefault(leftBtn, 0) + 1);
        map.put(rightTop, map.getOrDefault(rightTop, 0) + 1);
        map.put(leftTop, map.getOrDefault(leftTop, 0) + 1);
        map.put(rightBtn, map.getOrDefault(rightBtn, 0) + 1);
    }
}
```
...
<img src="301-400.assets/image-20210329171813616.png" alt="image-20210329171813616" style="zoom:50%;" />
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210511172448489.png" alt="image-20210511172448489" style="zoom:50%;" />
    ...
    public int longestSubstring(String s, int k) {
        ...
        if(splitPoint.size() == 0){
            ...
            for(int i = 0;  i< splitPoint.size(); i++){
                int nextIndex = splitPoint.get(i);

                if(index == 0)
                    strs.add(s.substring(0, nextIndex));
                else
                    strs.add(s.substring(index + 1, nextIndex));
                index = nextIndex;

                if(i == splitPoint.size() - 1)
                    strs.add(s.substring(nextIndex + 1));
            }
...
public int longestSubstring(String s, int k) {
    ...
    private int getRes(String s, int k){
        int[] map = new int[26];
        for(char ch : s.toCharArray())
            map[ch - 'a']++;
...
<img src="301-400.assets/image-20210330165351682.png" alt="image-20210330165351682" style="zoom:50%;" />
...
public int maxRotateFunction(int[] nums) {
        ...
        */
        ...
        for(int i = 1; i < nums.length; i++){
            ...
            func = func + sum - nums.length * nums[nums.length - i];
            res = Math.max(func, res);
        }
...
<img src="301-400.assets/image-20210330205306474.png" alt="image-20210330205306474" style="zoom:50%;" />
...
<img src="301-400.assets/image-20210330200456000.png" alt="image-20210330200456000" style="zoom:50%;" />
...
private Map<String, HashMap<String, Double>> g = new HashMap<>();
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        for(int i = 0; i < equations.size(); i++){
            ...
            g.computeIfAbsent(y, m -> new HashMap<String, Double>()).put(x, 1.0 / k);
        }
...
<img src="301-400.assets/image-20210331091159557.png" alt="image-20210331091159557" style="zoom:50%;" />
...
public int findNthDigit(int n) {
        ...
        while(ln > 0){
            long roundDigits = getRoundDigits(digits);
        ...
        long whichNumber = (ln - 1) / (digits);
        long whichPos    = (ln - 1) % (digits);
        long startNumber = getStartNumber(digits);
    ...
    private long getRoundDigits(long digits) {
        ...
        return (Long.parseLong(end.toString()) - getStartNumber(digits) + 1) * digits;
    }
