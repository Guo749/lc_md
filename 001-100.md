...
func getRes(dividend int, divisor int) int{
	if divisor < dividend{
		return 0
	}
	...
	for ; tb + tb < 0 && dividend < tb + tb; {
		tb += tb
		count += count
	}

	return count + getRes(dividend -  tb, divisor)
}
...
<img src="001-100.assets/image-20210923150650796.png" alt="image-20210923150650796" style="zoom:50%;" />
...
public:
    vector<int> searchRange(vector<int>& nums, int target) {
        ...
        left = 0, right = ans;
        while(left <= right){
            int myMid = (left + right) / 2;
            if(nums[myMid] == nums[ans]){
                leftAns = myMid;
                right = myMid - 1;
            }else { // nums[myMid < mid
                left = myMid + 1;
...
<img src="001-100.assets/image-20210717101834902.png" alt="image-20210717101834902" style="zoom:50%;" />
...
<img src="001-100.assets/image-20210325153128671.png" alt="image-20210325153128671" style="zoom:50%;" />
...
<img src="001-100.assets/image-20210707093346965.png" alt="image-20210707093346965" style="zoom:50%;" />
    ...
    vector<vector<int>> merge(vector<vector<int>>& intervals) {
     ...
     sort(intervals.begin(), intervals.end(), [](vector<int>& v1, vector<int>& v2){return v1[0] < v2[0]; });
        ...
        while(right < intervals.size()){
            ...
            while(right < intervals.size() && intervals[right][0] <= rightBound){
                rightBound = max(rightBound, intervals[right][1]);
                right++;
            }
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210513170518799.png" alt="image-20210513170518799" style="zoom:50%;" />
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210510123030533.png" alt="image-20210510123030533" style="zoom:50%;" />
  ...
  //  
	public int removeDuplicates(int[] nums) {
        ...
        while(right < len){
            ...
            for(int i = 0; i < Math.min(right - left, 2); i++)
                nums[index++] = nums[left];
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210512173652701.png" alt="image-20210512173652701" style="zoom:50%;" />
...
<img src="/Users/admin/Library/Application Support/typora-user-images/image-20210511164601342.png" alt="image-20210511164601342" style="zoom:50%;" />
...
<img src="001-100.assets/image-20210519080941587.png" alt="image-20210519080941587" style="zoom:50%;" />
...
func restoreIpAddresses(s string) []string {
	...
	temp := ""
	backtrack(&res, s, 0, temp, 0)

	return res
}

func backtrack(res *[]string, s string, index int, path string, num int){
	if index == len(s){
		if num == 4{
			*res = append(*res, path)
		}
	...
	for i := index; i < len(s); i++{
		...
		if number, err := strconv.Atoi(frac); err == nil{
			if frac[0] == '0' && len(frac) != 1{
				continue
			}
			...
			if 0 <= number && number <= 255{
				...
				backtrack(res, s, i + 1, path, num + 1)
...
func getRes(start int, end int) []*TreeNode{
	if start > end{
		return []*TreeNode{nil}
	}
	...
	for i := start; i <= end; i++{
		leftTrees  := getRes(0, i - 1)
		rightTrees := getRes(i + 1, end)
